...
export function getBrandBadgeHtml(brand, isSelected = false, size = 'sm') {
  ...
  // HYUNDAI - Official Slanted Oval with Dynamic Italic 'H' SVG
  if (id === 'hyundai' || name.includes('hyundai')) {
    return `
      <span class="${sizeClass} rounded-md bg-sky-950 border border-sky-600/60 text-sky-400 flex items-center justify-center p-0.5 shadow-sm" title="Hyundai">
        <svg viewBox="0 0 100 65" class="w-full h-full" fill="none" stroke="#38bdf8" stroke-width="6.5" stroke-linecap="round">
          <ellipse cx="50" cy="32" rx="46" ry="28" transform="rotate(-6 50 32)" />
          <path d="M33 16 C33 24 37 40 35 48 M67 16 C67 24 63 40 65 48 M34 32 C45 38 55 38 66 32" />
        </svg>
      </span>
    `;
  }
  ...
  // MAHINDRA - Official New "Twin Peaks" Chrome Wings Emblem SVG
  if (id === 'mahindra' || name.includes('mahindra')) {
    return `
      <span class="${sizeClass} rounded-md bg-gradient-to-b from-red-900 to-slate-950 border border-red-500/40 text-white flex items-center justify-center p-0.5 shadow-sm" title="Mahindra (Twin Peaks)">
        <svg viewBox="0 0 100 85" class="w-full h-full">
          <!-- Left Wing Peak -->
          <path d="M12 70 C 18 52, 28 32, 45 12 C 43 28, 38 48, 44 65 C 36 68, 24 71, 12 70 Z" fill="#ffffff" />
          <!-- Right Wing Peak -->
          <path d="M88 70 C 82 52, 72 32, 55 12 C 57 28, 62 48, 56 65 C 64 68, 76 71, 88 70 Z" fill="#ffffff" />
          <!-- Center Connecting Chrome Bridge -->
          <path d="M44 65 L 50 52 L 56 65 Z" fill="#cbd5e1" />
        </svg>
      </span>
    `;
  }
